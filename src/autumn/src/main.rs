...
use util::variables::{CONFIG, HOST, LOCAL_STORAGE_PATH, USE_S3, GIT_COMMIT};
        ...
        App::new()
            .wrap(
                Cors::default()
                    ...
                    .allowed_headers(["X-Session-Token", "X-Client-Token"])
            ...
            .route("/health", web::get().to(routes::index::get_health))
    })
    .bind(HOST.clone())?
    .run()
    .await
