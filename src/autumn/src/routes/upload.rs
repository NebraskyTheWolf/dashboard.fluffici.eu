        ...
        let metadata = match s {
            ...
            _ => {
                if inspect(&buf).is_text() {
                    ...
                    if *USE_CLAMD {
                        let scan_response = fluffici_clamav_client::scan_buffer_tcp(&buf, CLAMD_HOST.to_string(), None).unwrap();

                        let file_scan = fluffici_clamav_client::clean(&scan_response).unwrap();
                        if !file_scan {
